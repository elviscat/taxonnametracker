<?php
  ...
  //August 24, 2011 Wednesday:: Code Logic modification
  //May 03, 2012 Thursday:: Code Logic modification
  ...
  $table .= "<form>\n";
  $table .= "\t<fieldset>\n";
  $table .= "\t\t<legend><h3>Taxonomic Revision Proposal</h3></legend>\n";
  ...
  */
  ...
  if( mysql_num_rows($result) > 0 ){  
    while ( $nb2 = mysql_fetch_array($result) ) {
        ...
        //Add on July 06, 2012 Friday:: Add the status remark to let user can easily recognize this proposal
        $table .= "<h3>Status: ".$pstate."</h3>";//Title
        
        
        $table .= "<h3>Topic: ".$nb2[1]."</h3>";//Title
		...
		$table .= "<B><font color=\"blue\"><a href=\"viewpostlist.php?username=".$post_author_username."\">".$post_author_name."</a></font></b><BR />\n";//Posted by Author
		
		
		
		$table .= "<font color=\"gray\">".$nb2[3]."</font><BR />\n";//Time
		$table .= "<font color=\"black\">".$nb2[2]."</font><BR />\n";//Post Content
        //$uid is here!
        $uid = $_SESSION['uid'];
  ...
  */
  ...
  //Here added on April 12, 2010 Monday refactoring the code
	  ...
	  //$table .= "<p>No post.</p>\n"; //no records here
	  $table .= "<p>No taxonomic change proposal here!</p>\n"; //Modified on July 05, 2012 Friday
  ...
  $sql3 = "SELECT COUNT(*) FROM comment WHERE crefpid= '".$crefpid."' ".$sql_comment_where_clause;
  //echo "sql3 is ".$sql3;
  ...
  $table .= "<form>\n";
  $table .= "\t<fieldset>\n";
  $table .= "\t\t<legend><h3><B>Comments: <font color=\"blue\">".$commentCount."</font></B></h3></legend>\n";
  ...
  */
  ...
  if( mysql_num_rows($result) > 0 ){  
    ...
    while ( $nb2 = mysql_fetch_array($result) ) {
      ...
      $table .= "Said by <font color=\"blue\"><a href=\"viewpostlist.php?username=".$commentator_username."\">".$commentator_name."</a></font><BR />";//Posted by Author
  ...
  if($is_names_committee_member == True || $role == "admin"){
	...
	$table .= "<form>\n";
	$table .= "\t<fieldset>\n";
	$table .= "\t\t<legend><h3><B>Review Opinions: <font color=\"blue\">".$commentCount."</font></B></h3></legend>\n";
          ...
          if( $pstate == "Submitted" ){//Just show up the general Comment Interface
            ...
            */
            ...
            if( $is_user_auth == True && $is_voted == False ){
				...
				$vote_form = "<form name =\"vote\" method =\"post\" action =\"names_com_vote2.php\">";
				$vote_form .= "\t<fieldset>\n";
				$vote_form .= "\t\t<legend><h3>Vote Interface</h3></legend>\n";
				
				
				$vote_form .= "<label>User can cast their vote!</label>\n";
				//$vote_form .= "<input type = \"hidden\" name = \"is_voted\" id = \"is_voted\" value = \"1\" />";
				$vote_form .= "<input type = \"hidden\" name = \"pid\" id = \"pid\" value = \"".$pid."\" />";
				$vote_form .= "<input type = \"hidden\" name = \"uid\" id = \"uid\" value = \"".$uid."\" />";
				$vote_form .= "<input type = \"radio\" name = \"vote_opinion\" id = \"vote_opinion\" value = \"Approve\" >Approve";
				$vote_form .= "<input type = \"radio\" name = \"vote_opinion\" id = \"vote_opinion\" value = \"Disapprove\" >Disapprove";
				$vote_form .= "<input type = \"radio\" name = \"vote_opinion\" id = \"vote_opinion\" value = \"Abstain\" >Abstain";
				$vote_form .= "<br>\n";
				$vote_form .= "Write down your reason:<br> <textarea name = \"vote_desc\" id = \"vote_desc\" rows=\"10\" col=\"100\">Vote Description</textarea>";
				$vote_form .= "<br>";
				$vote_form .= "<input type=\"submit\" name=\"submit\" value =\"Cast Your Vote\">";
				...
				echo "<form>\n";
				echo "\t<fieldset>\n";
				echo "\t\t<legend><h3>Vote End</h3></legend>\n";
            	...
            	if(mysql_num_rows($result_vote_opinion) > 0){
            		while ( $nb_vote_opinion = mysql_fetch_array($result_vote_opinion) ) {
						...
						echo "Your vote: <b>".$vote_opinion."</b><br>\n";
						echo "Your vote opinion: <b>".$vote_desc."</b><br>\n";
						//echo "<font color=\"red\">Do we need Statistic Information?</font><br>\n";
					}
				}
				echo "\t</fieldset>\n";
            ...
            if( $role == "admin"){
              ...
              echo "<form name =\"final_decision\" method =\"post\" action =\"final_decision2.php\">\n";
  			  echo "\t<fieldset>\n";
  			  echo "\t\t<legend><h3>Administrator's final decision</h3></legend>\n";
              ...
              $sql_vote = "SELECT * FROM committee_member WHERE rank_level != 'Admin' AND join_status = 'Accept' AND ref_pid ='".$pid."'";
              ...
              if(mysql_num_rows($result_vote ) > 0){
                while ( $nb_vote  = mysql_fetch_array($result_vote ) ) {
                  //
                  $total_members++;
                  if( $nb_vote[6] == "Approve" ){
                    $total_yes++;
                  }elseif( $nb_vote[6] == "Disapprove" ){
                    $total_no++;
              ...
              echo "<table border=\"1\">\n";
              echo "<tr>\n";
              echo "<td>Number of Names Committee Members</td>\n";
              echo "<td>Number of Vote \"Approve\"</td>\n";
              echo "<td>Number of Vote \"Disapprove\"</td>\n";
              echo "<td>Number of Vote \"Abstain\"</td>\n";
              echo "<tr>\n";
              echo "<tr>\n";
              echo "<td><b>".$total_members."</b><br>\n";
              echo "<td><b>".$total_yes."</b></td>\n";
              echo "<td><b>".$total_no."</b></td>\n";
              echo "<td><b>".$total_abstain."</b></td>\n";
              echo "<tr>\n";
              echo "</table>\n";
              
              echo "<br><b>Final Decision</b><br>\n";
              //$vote_form = "<form name =\"final_decision\" method =\"post\" action =\"final_decision2.php\">";
              $vote_form .= "<input type = \"hidden\" name = \"pid\" id = \"pid\" value = \"".$pid."\" />";
              $vote_form .= "<input type = \"hidden\" name = \"uid\" id = \"uid\" value = \"".$uid."\" />";
              $vote_form .= "<input type = \"radio\" name = \"final_decision\" id = \"final_decision\" value = \"Approve\" >Approve";
              $vote_form .= "<input type = \"radio\" name = \"final_decision\" id = \"final_decision\" value = \"Disapprove\" >Disapprove";
              
              $vote_form .= "<br /><br />\n";
              $vote_form .= "Write down your decision summary:<br> <textarea name = \"decision_summary\" id = \"decision_summary\" rows=\"15\" cols=\"75\">Decision Summary</textarea>";
              $vote_form .= "<br>";
			...
			echo "<form>\n";
			echo "\t<fieldset>\n";
			echo "\t\t<legend><h3>Administrator's Decision Summary:</h3></legend>\n";
            
            
            //echo "<b>This posted proposed change has been closed and end its conversation now!</b><br>\n";
            echo "This posted proposed change has been closed and end its conversation now.<br>\n";
            ...
            if(mysql_num_rows($result_final_decision ) > 0){
              while ( $nb_final_decision  = mysql_fetch_array($result_final_decision ) ) {
                ...
                echo "Final decision: <b>".$final_decision."</b><br>\n";
                echo "Decision summary: <b>".$decision_summary."</b><br>\n";
              }
            }
			echo "\t</fieldset>\n";
			echo "</form>\n";
