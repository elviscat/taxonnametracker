<?php 
  ...
  //May 2, 2012 Wednesday:: Modification
  ...
  */
  ...
  if( $ptype == "type1" ){//If ptype equal "type1", then include all new species information in the following...
    ...
    //exit();
    $sql_insert_new = "INSERT INTO `taxon_name_tracker`.`slist` (`sid` ,`comes_from`, `sfamily` ,`sgenus` ,`sspecies` ,`sauthor` ,`sloc` ,`scommon_name` ,`scnam1` ,`scnam2` ,`scnam3` ,`synonym_of` ,`is_valid` ,`footnote`) VALUES ";
    ...
    //echo "\$species_id is : ".$species_id."<br />\n";
    $ptitle = "This Species <i>".taxon_name_without_level('species', $prefsid)."</i> is synonymized with <i>".taxon_name_without_level('species', $species_id)."</i>";
    ...
    //$p_operation_script_1 = "UPDATE `taxon_name_tracker`.`slist` SET `synonym_of` = \'$species_id\' WHERE sid =\'".$prefsid."\'";
    //$p_operation_script_2 = "";
    //echo "\$p_operation_script_1 is <b>".$p_operation_script_1."</b><br />\n";
    //echo "\$p_operation_script_2 is <b>".$p_operation_script_2."</b><br />\n";
    //exit();
    $sql_insert_new = "INSERT INTO `taxon_name_tracker`.`slist` (`sid`) VALUES ";
    $sql_insert_new .= "('$maxSid')";
	//get slist's column name
	//for loop
	mysql_query($sql_insert_new);
	$sql_update = "UPDATE slist SET comes_from = (SELECT come_from FROM slist WHERE sid= '".$prefsid."')";
  ...
  */
  ...
  if( $preflv == "" && $prefsid == ""){
    ...
    $insert_sql = "INSERT INTO post (`pid` ,`ptitle` ,`pcontent` ,`pcredate` ,`prefuid` ,`pref_taxa` ,`ptype` ,`pcount` ,`ptag` ,`pcategory` ,`pstate` ,`pexpiration` ,`pattachment_id` ,`p_operation_script` ,`p_final_decision`) VALUES ";
    ...
    $insert_sql .= "('".$maxPid."', '".$ptitle."', '".html_entity_decode($pcontent)."', '".$date."', '".$prefuid."', '".$preflv.":".$prefsid.";".$preflv.";".$maxSid."', '".$ptype."', '0', '', '', 'Submitted', '".$expirationtime."', '".$attachment."', '".$p_operation_script_1.":".$p_operation_script_2."', 'Pending')";
    //echo "\$insert_sql is <b>".$insert_sql."<b><br>\n";
