<?php 
  ...
  for($i = 0; $i < sizeof($array_selected_users_output);$i++){      
    $sql_set_to_com_mem =  "INSERT INTO committee_member (`cmid`,`ref_uid`,`ref_pid`,`invited_date`,`join_status`,`rank_level`)";
    $sql_set_to_com_mem .= " VALUES ('$max_committee_member_id','$array_selected_users_output[$i]','$pid','$invited_date','Pending','Member')";
    ...
    */
    ...
    if(mysql_query($sql_set_to_com_mem)){
      //February 26, 2011 Saturday:: Add logic that change the viewpost interface state from Submitted to Under Review while names committee member(s) has been initially set up
      //Update TABLE::post::pstate from Submitted to Under Review:: open viewpost interface to let names committee member(s) write down some review opinion and recommended decision
      $sql_post_pstate_update = "UPDATE post SET pstate ='Under Review' WHERE pid ='".$pid."'";
