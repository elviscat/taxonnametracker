...
**  version3:
**  07/07/2012 Saturday:: Change $from_email_name
*
*******************************/
// ./??????
// ../??????
...
function related_taxa_from_pid($lv, $table_name, $pid){
	//related_taxa_from_pid('species', 'slist', 10)
	...
	if(mysql_num_rows($result) > 0){
		while ( $nb = mysql_fetch_array($result) ) {
			if($nb['footnote'] != ''){
				//echo "\$sid is ".$nb[0]." \$pid is ".$pid." \$nb['footnote'] is ".$nb['footnote']." <br />\n";
				$footnote_array = explode(";", $nb['footnote']);
				for($i= 0; $i < sizeof($footnote_array); $i++){
					//echo "\$footnote_array[$i] is ".$footnote_array[$i]." == ".$pid."<br />\n";
					if( $footnote_array[$i] == $pid ){
						//$related_taxa .= $nb[0].";";
						
						//echo "Yes<br />\n";
						
						$related_taxa_link .= "<a href=\"viewtaxon.php?lv=".$lv."&id=".$nb[0]."\">".$nb[0]."</a><br>";
	...
	//$related_taxa = substr($related_taxa, 0, -1);
	
	//return $related_taxa;
	return $related_taxa_link;
}



function footnote($lv, $id){

  $sql_footnote = "";
  if($lv == "family"){
    $sql_footnote = "SELECT footnote FROM flist WHERE fid =".$id;
  }elseif($lv == "genus"){
    $sql_footnote = "SELECT footnote FROM glist WHERE gid =".$id;
  }elseif($lv == "species"){
    $sql_footnote = "SELECT footnote FROM slist WHERE sid =".$id;
  }
  $result_footnote = mysql_query($sql_footnote);
  $footnote = "";
  if(mysql_num_rows($result_footnote) > 0){
    while ( $nb = mysql_fetch_array($result_footnote) ) {
      $footnote = $nb[0];
    }
  }
  return $footnote;
...
$admin_password = "xu3r85";
//$admin_email = "cypriniformes@gmail.com";
//$admin_email = "hwu5@slu.edu";
$admin_email = "maydenrl@slu.edu";
$admin_name = "Richard Mayden";

//$from_email_name = "Cyber Nomenclatorial Process Platform of North American Freshwater Fishes System Administrator";
//$from_email_name = "Taxon Tracker Administrator";
$from_email_name = "Taxon Name Tracker Administrator";//Modified on July 07, 2012 Saturday

//$application_caption = "<font size=\"6\">Taxon Tracker</font><sub>Beta</sub>";
//$application_caption = "Taxon Name Tracker Beta";
$application_name = "Taxon Name Tracker Beta";
